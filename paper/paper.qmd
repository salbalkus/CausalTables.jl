...
with a user-specified causal structure and computing ground
...
emerge---with examples including, for estimation, `TMLE.jl` [@TMLE.jl] and `CausalELM.jl`
...
designed to support the evaluation and comparison of their
...
from it, and compute or approximate the underlying true values of several 
common causal effect parameters.
...
`CausalTables.jl` provides functions to approximate ground truth
values of common causal estimands such as the ATE (using `ate`). These functions 
...
supports other estimands. Consider the following SCM, in which the
...
model (such as a general linear model) and using this to predict the outcome 
$Y$ under the modified treatment policy $d(A, W;\delta): A \to A + \delta$; 
the average difference of these predictions from the observed $Y$ yields the 
APE. We can use `CausalTables.jl` to see what the output  of such a procedure 
would have been had we known the true value of the outcome regression 
