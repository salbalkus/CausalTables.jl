...
```jldoctest titanic; output = false, filter = r"(?<=.{11}).*" # only take the first 12 characters of output
using CausalTables
...
```jldoctest karateclub; output = false, filter = r"(?<=.{11}).*"
using CausalTables
using MLDatasets
using Graphs
...
```jldoctest karateclub; filter = r"(?<=.{17}).*"
